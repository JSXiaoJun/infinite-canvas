...
import { createVideoGenerationTask, pollVideoGenerationTask, storeGeneratedVideo, videoTaskPollingOptions, type VideoGenerationTask } from "@/services/api/video";
...
export default function VideoPage() {
    ...
    const pollGenerationLog = async (log: GenerationLog, configOverride?: AiConfig, agentTaskId?: string) => {
        ...
        const polling = videoTaskPollingOptions(log.task);
        try {
            for (let attempt = 0; attempt < polling.maxAttempts; attempt += 1) {
                ...
                if (attempt === polling.maxAttempts - 1) throw new Error(t("videoWorkbench.timeout"));
                await delay(polling.intervalMs);
            }
        } catch (error) {
            const errorMessage = error instanceof Error ? error.message : t("workbench.generationFailed");
