...
+ [优化] 配置页支持手动获取并持久缓存 YYAPI 视频模型能力，匹配 `yyapi.cloud` 视频模型时按缓存覆盖清晰度和时长并避免不支持的参数组合。
+ [修复] 生图工作台请求始终使用当前所选渠道中的完整模型名称，避免带服务前缀的模型被错误替换为同名基础模型。
+ [修复] 视频任务创建或查询请求达到超时时间时明确显示超时，不再误报为跨域错误。
+ [修复] 画布生成配置始终传入所有已连接的图片、视频、音频和文本素材，`@` 仅用于在提示词中插入素材标签。
+ [新增] 视频设置清晰度预设新增 768p。
+ [新增] 画布节点右键菜单支持清空当前节点结果并保留提示词与生成配置。
+ [调整] 每次推送任意分支后自动构建并发布应用与文档 Docker 镜像到当前仓库所属的 GHCR 命名空间。
+ [调整] 配置渠道恢复仅保留默认渠道，移除内置 YYAPI 视频渠道；视频接口兼容和本地参考素材 R2 上传能力保留。
+ [新增] 渠道配置新增 Sora 协议，以 JSON 视频任务格式兼容任意模型和中转地址。
+ [修复] 兼容 OpenAI 图片接口返回的对象、字符串化 JSON 及带 MIME 前缀的 Base64 图片，并直接解码 data URL 后写入本地图片存储，避免有效响应被判定为全部生成失败。
+ [修复] 视频任务完成后不再因远程视频下载或元信息读取无限等待而持续显示生成中，并兼容更多视频 URL 返回字段与 Markdown 链接格式。
+ [修复] 生成配置并发生成视频时正确识别包装响应与多种失败终态，并避免重复触发或旧任务收尾遗留永久加载节点。
